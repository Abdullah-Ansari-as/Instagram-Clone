...
function CommentDialog({ open, setOpen, openCommentDialog, setOpenCommentDialog, isFollowing, followUnfollowHandler }) {
  ...
  const handleSubmitComment = async (event) => {
    ...
    try {
      ...
      const res = await axios.post(`https://insta-clone-abd.up.railway.app/api/v1/posts/${selectedPost?._id}/comment`, { text }, {
        headers: {
  ...
  const deletePostHandler = async () => {
    try {
      const res = await axios.delete(`https://insta-clone-abd.up.railway.app/api/v1/posts/delete/${selectedPost?._id}`, { withCredentials: true })
      if (res.data.success) {
  ...
  const bookmarkHandlerr = async () => {
    try {
      const res = await axios.get(`https://insta-clone-abd.up.railway.app/api/v1/posts/${selectedPost?._id}/bookmark`, { withCredentials: true });
      if (res.data.success) {
  ...
  const likeDisLikHandler = async () => {
		try {
			...
			const res = await axios.get(`https://insta-clone-abd.up.railway.app/api/v1/posts/${selectedPost?._id}/${action}`, { withCredentials: true });
			if (res.data.success) {
